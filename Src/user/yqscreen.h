...
void comscreen(uint8_t* str,short int length);  //发送 指令至液晶屏
extern uint16_t touchScreenDataFlag;
extern uint16_t touchScreenDataCount;  // 串口2接收 字节 计数
...
extern volatile uint8_t scrdis[5][50];  //根据币种 确定图片
...
extern uint8_t Disp1_Indexpic[Number_IndexpicB];         //MCU控制图片寄存器数据  切换 屏图片 指令数组
extern uint8_t Disp_Indexpic[Number_IndexpicA][Number_IndexpicB];         //MCU控制图片寄存器数据  切换 屏图片 指令数组

extern volatile uint16_t prepic_num; 	//记录当前的 图片号 所显示的
int16_t test_repete(void);   

#define TOUCH_SCREEN_DATA_BUF_LEN 32 //触摸屏数据帧最大长度
